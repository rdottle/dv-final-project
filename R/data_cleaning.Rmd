...
# Bronx 
dtm_sum_bx <- doc_sum_bx %>% DocumentTermMatrix()
tdm_sum_bx <- doc_sum_bx %>% TermDocumentMatrix()


# Staten Island

doc_sum_si <- Corpus(VectorSource(si_listings$summary)) %>%
  tm_map(removePunctuation) %>% tm_map(removeNumbers) %>% tm_map(tolower) %>%
  tm_map(removeWords, stopwords("english")) %>% tm_map(PlainTextDocument) 
...
pal_2 <- wes_palette("Moonrise3", type = "continuous")
...
qn_list_top_20 <- FreqMat_mn %>% filter(Freq > 3600)
...
#calculate the top frequency words listings
...
findAssocs(dtm_sum, terms = "sketchy", corlimit = 0.1)
