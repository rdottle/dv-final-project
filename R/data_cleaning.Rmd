...
findFreqTerms(dtm_sum, lowfreq = 900)
...
# Brooklyn
freq <- colSums(as.matrix(dtm_sum)) 
len_dtm <- freq %>% length()

# Word matrix length (no words included in word frequency) 
len_dtm
wfreq <- as.data.frame(freq) %>% subset(freq > 2000)
...
wfreq_mn <- as.data.frame(freq_mn) %>% subset(freq_mn > 2000)
